...
pub struct EptResponder {
  ...
  pub cache: Option<UpdateResponseCacheMessage>,
  pub respond: Recipient<UpdateResponseCacheMessage>,
}
    ...
    let block = async {
      let message = {
        if let None = self.cache {
          self.daemon.send(RefreshMessage::Refresh { force: true }).await??
        } else {
          self.daemon.send(n.into()).await??
        }
      };

      Result::<UpdateResponseCacheMessage, anyhow::Error>::Ok(message)
    };
