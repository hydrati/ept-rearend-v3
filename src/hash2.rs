...
impl IntegrityCache {
  ...
  pub fn compute<P: AsRef<Path>>(&self, method: IntegrityMethod, path: P) -> anyhow::Result<Integrity> {
    match method {
...
mod tests {
  ...
  #[test]
  fn compute_hash_blake3() -> anyhow::Result<()> {
    let cache = IntegrityCache::new();
    let integrity = cache.compute(crate::class::IntegrityMethod::Blake3, r#"C:\Users\Hydrogen\Downloads\Firless.3.0.0+stable_amd64.iso"#)?;
    println!("{:#?}", integrity);
    Ok(())
  }

  #[test]
  fn compute_hash_blake3_times() -> anyhow::Result<()> {
    for _ in 0..10 {
      compute_hash_blake3()?;
    }
  ...
  #[test]
  fn compute_hash_sha256() -> anyhow::Result<()> {
    let cache = IntegrityCache::new();
    let integrity = cache.compute(crate::class::IntegrityMethod::Sha256, "./Edgeless_Beta_4.1.0.iso")?;
    println!("{:#?}", integrity);
    Ok(())
  }
}